...
| `@/utils/countries` | `COUNTRY_OPTIONS` 内置国家兜底与 `getCountryLabel()` 展示映射；新建国家下拉优先调用 `GET /api/config/country-options`，接口不可用时再降级到内置选项；订单编辑 / 信息匹配输入新国家时必须使用 `XX - 中文名`，后端会写入 `country_name_override` |
...
- 订单页写操作统一由 `data_biz:edit` 控制；新增订单编辑或批量导入能力时，必须同时维护权限可见性、后端 `require_permission(DATA_BIZ_EDIT)` 和对应单测。
