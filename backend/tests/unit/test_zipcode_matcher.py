...
def test_jp_number_ge_50_to_haiyuan() -> None:
    ...
    assert match_warehouses("640-8453", "JP", rules) == ["haiyuan"]  # 前2位 64 >= 50
    assert match_warehouses("500-0001", "JP", rules) == ["haiyuan"]  # 前2位 50 >= 50
    assert match_warehouses("050-0001", "JP", rules) == ["xiapu"]  # 前2位 05 < 50
    assert match_warehouses("100-0001", "JP", rules) == ["xiapu"]  # 前2位 10 < 50
...
def test_uk_string_eq() -> None:
    ...
    assert match_warehouses("SW1A 1AA", "UK", rules) == ["uk_a"]
    assert match_warehouses("EC1V 0HP", "UK", rules) == ["uk_b"]
    assert match_warehouses("NW1 7BD", "UK", rules) == []


def test_string_neq() -> None:
    rules = [_rule(1, "UK", 2, "!=", "SW", "default", priority=10, value_type="string")]
    assert match_warehouses("EC1V 0HP", "UK", rules) == ["default"]
    assert match_warehouses("SW1A 1AA", "UK", rules) == []


def test_string_contains_any_token() -> None:
    rules = [_rule(1, "UK", 4, "contains", "SW,EC", "uk_mix", priority=10, value_type="string")]

    assert match_warehouses("SW1A 1AA", "UK", rules) == ["uk_mix"]
    assert match_warehouses("EC1V 0HP", "UK", rules) == ["uk_mix"]
    assert match_warehouses("NW1 7BD", "UK", rules) == []
...
def test_string_not_contains_all_tokens() -> None:
    ...
    assert match_warehouses("NW1 7BD", "UK", rules) == ["uk_other"]
    assert match_warehouses("SW1A 1AA", "UK", rules) == []
    assert match_warehouses("EC1V 0HP", "UK", rules) == []
...
def test_prefix_too_short_skipped() -> None:
    ...
    assert match_warehouses("123", "JP", rules) == []
...
def test_between_single_segment_inclusive() -> None:
    ...
    # 边界都命中
    assert match_warehouses("000-1111", "JP", rules) == ["wh_west"]
    assert match_warehouses("270-0001", "JP", rules) == ["wh_west"]
    # 中间值命中
    assert match_warehouses("100-0001", "JP", rules) == ["wh_west"]
    # 越界未命中
    assert match_warehouses("271-0001", "JP", rules) == []
    assert match_warehouses("999-0001", "JP", rules) == []
...
def test_between_multi_segment_any_hit() -> None:
    ...
    assert match_warehouses("050-0001", "JP", rules) == ["wh_mix"]  # 命中第一段
    assert match_warehouses("600-0001", "JP", rules) == ["wh_mix"]  # 命中第二段
    assert match_warehouses("400-0001", "JP", rules) == []      # 两段都不命中
    assert match_warehouses("800-0001", "JP", rules) == []


def test_between_leading_zero_prefix() -> None:
    """前 3 位 '050' 应解析为整数 50,落在 000-270 范围内。"""
    rules = [_rule(1, "JP", 3, "between", "000-270", "wh_west", priority=10)]
    assert match_warehouses("050-0001", "JP", rules) == ["wh_west"]
...
def test_between_priority_still_first_wins() -> None:
    ...
    assert match_warehouses("100-0001", "JP", rules) == ["wh_a"]
    # 500 只落在第二段
    assert match_warehouses("500-0001", "JP", rules) == ["wh_b"]
