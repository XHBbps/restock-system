...
- **准确性诊断**：Step 2/3 区分“已知 0 库存”和“缺少库存记录”；有销量但没有库存/在途记录的 SKU+国家不会按 0 库存生成国家补货量，并在 `suggestion_item.calculation_warnings` 写入 `missing_inventory_record`。组合 SKU 若已有同国家组件信号但可组装数为 0，按已知 0 库存参与补货；海外库存页面组件现货按同海外仓折算，目标仓库为空的组件在途按国家汇总参与组合折算。SKU 级 `lead_time_days=0` 是有效值，不再回退全局货期。
...
- **单条重算与诊断**：新增 `POST /api/suggestions/{suggestion_id}/items/{item_id}/recalculate`，仅允许 `draft` 建议单条目重算 `sale_days_snapshot`、`urgent`、`restock_dates` 与 `calculation_warnings`，不改变 `country_breakdown`、`warehouse_breakdown` 或 `purchase_qty`。手工新增国家缺少 `sale_days` 时写入 `country_added_after_generation` 等诊断；补货日期统一回写建议单 `global_config_snapshot.demand_date`，历史建议缺少 `demand_date` 时保持空值。
...
- **订单信息匹配与编辑**：具备 `data_biz:edit` 权限时，订单页提供「编辑」入口和「信息匹配」Excel 导入弹窗，当前仅允许更新国家和邮编；编辑保存只提交实际变更字段，未修改时提示「没有修改内容」且不调用接口；导入国家使用二字码（如 `US`），若属于当前 `global_config.eu_countries` 则写入 `country_code='EU'` 并在 `original_country_code` 保留原码，非 EU 国家会清空该审计字段；邮编无格式校验，允许 `11` 等普通文本，也允许留空以清空原值；导入校验通过时展示命中订单号、更新字段和邮编清空提示；导入校验失败时仅展示错误数量和「下载错误文件」，可通过 `POST /api/data/order-info-match/error-report` 基于原文件下载追加「错误原因」列的 Excel；确认导入走 `POST /api/data/order-info-match/apply-task` 创建 `order_info_match_apply` 后台任务，弹窗内复用 `TaskProgress` 展示「已处理 N / 总数 M」，关闭后可通过活跃任务接口恢复进度；人工编辑后的订单由 `manual_edit_locked` 保护，后续同步不覆盖这些字段。店铺、平台、金额、币种、履约渠道、下单时间、最后更新时间等字段仅保留只读展示，不再允许通过订单编辑或信息匹配导入修改。
...
- **同仓口径**：组件 SKU 必须在同一个海外仓内组合，不跨海外仓、不跨国家组合；内部使用 `third_party:{id}` 作为海外仓计算键，避免与赛狐 `warehouse.id` 冲突。
- **在途合并**：海外库存页面组件现货会与有目标仓的组件在途一起进入同仓折算；目标仓为空的组件在途仍按国家级池重新折算，并覆盖该国家同仓结果，避免重复计数。
- **保持不变**：海外库存页面直接录入的商品 SKU 仍按 `commodity_sku + country` 直接计入；赛狐 `inventory_snapshot_latest` 非国内仓现货仍不参与 Step 2；Step 4 国内侧采购扣减逻辑不变。
- **测试覆盖**：`backend/tests/unit/test_engine_step2.py` 覆盖海外库存页面组件 SKU 折算、同海外仓约束、无国家海外仓过滤、海外组件现货与国家级在途合并，以及赛狐在途保留口径。
...
- **组件映射口径**：Step 2 的 SKU 映射不再通过赛狐非国内仓现货库存折算海外现货；海外库存页面组件现货和在途组件信号可按原有组合规则折算为商品 SKU 库存。
...
- **编辑与重算**：PATCH 修改国家补货量、单条重新计算时，所有正补货国家的 `restock_dates` 统一回写建议单 `global_config_snapshot.demand_date`；历史建议若缺少 `demand_date`，不按当前日期或旧公式补造日期，保持空值兼容。
- **快照与 Excel**：新生成补货快照冻结后的 `restock_dates` 使用 `demand_date`；补货 Excel 元信息页、`SKU×国家`、`SKU×国家×仓库` 的“补货日期”均写 `demand_date`，历史快照和已导出 Excel 不回写。
- **前端展示**：当前补货建议计算依据中的“补货日期”优先读取 `calculation_inputs_snapshot.demand_date`，国家明细中的 `restock_date` 作为同口径兼容字段；旧建议缺少计算依据时仍展示“历史建议缺少完整计算依据”。
...
- **后台任务**：新增 `order_info_match_apply` TaskRun job，解析 Excel 后复用订单信息匹配写库逻辑；写入阶段通过 `step_detail="已处理 N / 总数 M"` 输出确定型进度，成功时写 `result_summary="已更新 X 条订单"` 与 `result_payload.updatedOrderCount`，失败时由 worker 写入明确错误。
- **API 与并发**：新增 `POST /api/data/order-info-match/apply-task?fields=...` 与 `GET /api/data/order-info-match/apply-task/active`；同一时间只允许一个活跃导入任务，新请求若命中 pending/running 任务会返回 `{ existing: true, taskId }`，前端复用已有进度。
- **前端交互**：`frontend/src/views/data/DataOrdersView.vue` 的「确认导入」改为提交后台任务并在弹窗内展示 `TaskProgress`；弹窗关闭或页面刷新后重新打开会查询活跃任务恢复进度，任务成功后自动刷新国家选项和订单列表。
- **测试**：补充 `backend/tests/unit/test_order_edit_service.py`、`backend/tests/unit/test_data_orders_api.py`、`frontend/src/views/__tests__/DataOrdersView.test.ts` 与 `frontend/src/components/TaskProgress.test.ts`，覆盖任务创建、活跃任务复用、批次进度回调、进度解析和前端任务视图。
...
- **后端导入口径**：`backend/app/services/order_edit.py` 在信息匹配 `preview / apply / error-report` 解析时读取一次 `global_config.eu_countries`，国家列若为当前 EU 成员国则写入 `country_code='EU'`，并把原始成员国写入 `original_country_code`；非 EU 国家写入标准化二字码并清空 `original_country_code`，导入字面 `EU` 时保持 `country_code='EU'` 且不写原始成员国。
- **测试**：补充 `backend/tests/unit/test_order_edit_service.py` 与 `frontend/src/views/__tests__/DataOrdersView.test.ts`，覆盖 EU 成员国归并、非 EU 清空原始国家、字面 `EU` 幂等，以及失败态不展示逐行原因。

### 3.118 订单信息匹配错误文件导出（2026-05-13）
- **后端接口**：`backend/app/api/data.py` 新增 `POST /api/data/order-info-match/error-report?fields=...`，请求体沿用原 Excel 文件；后端复用 `backend/app/services/order_edit.py` 的信息匹配校验逻辑，只生成下载流，不写入数据库、不保存临时文件。
- **Excel 生成规则**：错误文件保留原 active sheet 的既有内容和样式，在最右侧追加「错误原因」列；行级错误写入对应原始行，同一行多条错误用 `；` 合并，错误原因单元格使用红色字体；空文件或无法读取的非 Excel 文件仍返回普通校验错误。
- **前端交互**：`frontend/src/views/data/DataOrdersView.vue` 在校验失败结果中新增「下载错误文件」按钮，调用 `frontend/src/api/data.ts` 的 `downloadOrderInfoMatchErrorReport()` 并通过 `triggerBlobDownload()` 下载 `订单信息匹配错误原因.xlsx`；校验成功时不显示该按钮。
- **测试**：补充 `backend/tests/unit/test_order_edit_service.py`、`backend/tests/unit/test_data_orders_api.py` 与 `frontend/src/views/__tests__/DataOrdersView.test.ts`，覆盖错误 workbook 内容、红色字体、同一行多错误合并、错误报告 endpoint 与前端下载调用。

### 3.117 订单页站点本地时间展示（2026-05-11）
- **后端展示字段**：`backend/app/api/data.py` 在订单列表和详情响应中新增 `purchaseDateLocal`、`lastUpdateDateLocal` 与 `displayTimezone`，由 `backend/app/core/timezone.py` 按 `marketplace_id` 优先、`country_code` 兜底推断展示时区；原始 `purchaseDate` / `lastUpdateDate` 继续保持北京时间存储与业务计算口径。
- **前端展示口径**：`frontend/src/views/data/DataOrdersView.vue` 的桌面列表、移动端卡片和详情弹窗优先渲染本地展示字段；缺少新字段时回退旧字段，兼容旧响应。列表排序仍使用 `purchaseDate`，日期筛选仍按北京时间自然日传给后端。
- **计算影响**：Step 1 销量速度、Step 5 分仓样本、补货建议和快照导出均未改变；本次只解决订单页“未来日期”的展示观感。
- **测试**：`pytest -p no:cacheprovider tests/unit/test_data_orders_api.py tests/unit/test_timezone.py`、`cmd /c npx vitest run src/views/__tests__/DataOrdersView.test.ts`、`cmd /c npx vue-tsc --noEmit` 与 `cmd /c npm run build` 通过。
...
- **引擎准确性**：`step2_sale_days` 与 `step3_country_qty` 不再把缺少库存/在途记录视为 0 库存；有销量但库存未知的国家不参与国家补货量计算，并在建议条目写入 `missing_inventory_record`。SKU 级 `lead_time_days=0` 按有效值参与紧急状态计算；用户可见补货日期现统一来自 `demand_date`。
...
- **建议单编辑与单条重算**：编辑补货国家后，`restock_dates` 使用建议单 `global_config_snapshot.demand_date`；新增 `POST /api/suggestions/{suggestion_id}/items/{item_id}/recalculate`，仅重算单条的 `sale_days_snapshot`、`urgent`、`restock_dates` 与 `calculation_warnings`，不重算补货量、采购量或仓库拆分。
...
- **PATCH 约束**：`backend/app/schemas/data.py` 的 `DataOrderPatch` 仅保留 `countryCode`、`postalCode`，通过 `extra="forbid"` 拒绝 `shopName`、`orderPlatform`、`orderTotalAmount`、`fulfillmentChannel` 等历史可写字段；列表和详情中的只读展示字段不变。
- **前端交互**：`frontend/src/views/data/DataOrdersView.vue` 的「信息匹配」字段勾选区仅展示「国家」「邮编」，编辑弹窗仅保留订单号、包裹号、包裹状态只读信息以及国家、邮编两个可编辑控件；`frontend/src/api/data.ts` 的 `DataOrderPatch` 类型同步收窄。
- **测试**：更新 `backend/tests/unit/test_order_edit_service.py`、`backend/tests/unit/test_data_orders_api.py` 与 `frontend/src/views/__tests__/DataOrdersView.test.ts`，覆盖被移除字段拒绝、国家/邮编允许、空邮编清空、导出模板参数和编辑差异 payload。

### 3.110 订单信息匹配导入校验结果紧凑化（2026-05-10）
- **接口响应**：`backend/app/schemas/data.py` 的 `OrderInfoMatchPreviewOut` / `OrderInfoMatchApplyOut` 新增 `matchedOrderIds`，由 `backend/app/services/order_edit.py` 在 workbook 无校验错误时从 `updates_by_order` key 返回命中订单号；有错误时返回空数组且 `matchedOrderCount` 保持 0。
- **前端交互**：`frontend/src/views/data/DataOrdersView.vue` 移除导入校验成功态的 `el-empty` 大块空白，改为紧凑结果面板；成功时展示「校验通过，可以确认导入」、命中订单号 tag、更新字段 tag，勾选「邮编」且无错误时展示「邮编校验通过，空值将清空原邮编」；失败时展示「校验未通过」和错误数量，并保留错误表格。
- **测试**：补充 `backend/tests/unit/test_order_edit_service.py`、`backend/tests/unit/test_data_orders_api.py` 与 `frontend/src/views/__tests__/DataOrdersView.test.ts`，覆盖命中订单号返回、成功/失败结果面板与确认导入启用条件。
...
- **跳过条件**：只有 `commoditySku` 与 `sellerSku` 同时为空、空字符串或纯空白时，才记录 `package_ship_item_skipped_missing_commodity_sku` 并跳过该明细；若本次没有任何有效明细，继续保留既有本地明细不删除。
- **影响范围**：不新增字段、不改接口 schema、不改前端展示字段；部署后重新执行 12 个月滚动订单同步即可回补此前 `commoditySku=NULL` 但 `sellerSku` 有值的 Wayfair 等平台明细。
- **测试**：补充 `backend/tests/unit/test_sync_order_list_eu.py`，覆盖 `commoditySku` 缺失时落库 `sellerSku`、`commoditySku` 有值时优先使用原值，以及双空时保留旧明细。
...
- **数据与导出保留**：API 类型中的 `restock_dates`、后端持久化、快照冻结和 `backend/app/services/excel_export.py` 补货 Excel “补货日期”列保持不变，用于历史追溯与 Excel 交付；当前紧急程度只由 `sale_days <= lead_time_days` 判定。
...
- **后端链路收缩**：`backend/alembic/versions/20260424_0100_drop_purchase_date_from_suggestions.py` 删除 `suggestion_item`、`suggestion_snapshot_item` 上的 `purchase_date`；`backend/app/engine/step6_timing.py` 当前仅保留 `urgent` 计算，`backend/app/engine/runner.py`、`backend/app/api/suggestion.py`、`backend/app/api/snapshot.py` 与相关 DTO 不再写入、存储或返回采购日期。
...
- **历史计算口径**：当时 `backend/app/engine/step6_timing.py` 新增 `compute_restock_dates()`，公式为 `restock_date[sku][country] = today + int(sale_days[sku][country]) − lead_time_days(sku)`；该用户可见日期口径已由 §3.123 取代，当前新建议统一使用 `demand_date`。
