<template>
  <div class="history-child">
    <el-table v-loading="loading" :data="rows" empty-text="暂无采购快照记录">
...
async function load(): Promise<void> {
  ...
  try {
    ...
    const suggestions = resp.items.filter((row) => row.procurement_snapshot_count > 0)
    const snapshots = await Promise.all(suggestions.map((row) => listSnapshots(row.id, 'procurement')))
